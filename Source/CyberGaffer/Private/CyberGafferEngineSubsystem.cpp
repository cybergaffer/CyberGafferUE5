...
static constexpr uint8 TargetMipSizeForTaskSphere = 128;
...
static uint8 GetTargetMipLevelIdForSphereTaskTexture(const uint32 width) {
	// check(width >= TargetMipSize);
	return static_cast<uint8>(log2(width / TargetMipSizeForTaskSphere));
}
...
void UCyberGafferEngineSubsystem::OnUpdateSceneCaptureContentsEnqueued(const FString serverIp, const uint32_t port, UTextureRenderTargetCube* texture) {
	...
	ENQUEUE_RENDER_COMMAND(HDRI2REALITY_ProcessEnvironment)(
		[texture, dataSender, serverIp, port](FRHICommandListImmediate& rhiCmdList)
		{
			...
			if (sideSize > TargetMipSize) {
				RenderMips(targetMip,mips,rhiCmdList);
			}
...
void UCyberGafferEngineSubsystem::OnUpdateSceneCaptureContentsEnqueued(const FString serverIp, const uint32_t port, UTextureRenderTarget2D* texture) {
	...
	ENQUEUE_RENDER_COMMAND(HDRI2REALITY_ProcessEnvironment)(
		[texture, dataSender, serverIp, port](FRHICommandListImmediate& rhiCmdList)
		{
			...
			uint8 targetMip = 0;
			if (sideSize > TargetMipSizeForTaskSphere) {
				targetMip = GetTargetMipLevelIdForSphereTaskTexture(sideSize);
			}
...
void UCyberGafferEngineSubsystem::RenderMips(uint8 targetMip,TArray<FTextureRHIRef>& mips, FRHICommandListImmediate& rhiCmdList)
{
